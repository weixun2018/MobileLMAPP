# Compare the differences between the original model and the fine-tuned model in answering psychological questions.
...
def load_model(model_path, device='cuda', force_auto_device_map=False):
    ...
    try:
        ...
        base_model = AutoModelForCausalLM.from_pretrained(
            ...
            device_map="auto" if force_auto_device_map else {"": device},
...
def compare_responses(queries):
    ...
    for i, query in enumerate(queries, 1):
        ...
        peft_response = chat(query)

        # Switch to original model to get response
        global peft_model
        temp_model = peft_model
        peft_model = original_model
        original_response = chat(query)
        peft_model = temp_model

        # Format result, including the query
        result = f"<问题{i}>: {query}\n<微调模型回复>：{peft_response}\n<原模型回复>：{original_response}\n\n-----------------------\n"
