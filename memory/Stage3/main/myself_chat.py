...
class MessageStore:
    ...
    def __init__(self, connection: sqlite3.Connection):
        ...
        self.batch_size = 8
        init_db(self.connection.cursor())
    ...
    def check_and_generate_summary(self):
        """检查是否需要生成新的总结"""
        total_messages = self.connection.execute("SELECT COUNT(*) FROM chat_history").fetchone()[0]
    ...
    def _generate_new_summary(self):
        """用上一次背景和最近对话生成新的背景"""
        last_summary = self.connection.execute(
            """
        ...
        recent_messages = self.connection.execute(
            """
        ...
        console.print("正在生成新的背景......", style="info")
        ...
        self.connection.execute("INSERT INTO chat_summary (summary) VALUES (?)", (summary,))
        self.connection.commit()
...
- 保持用户的第一人称视角"""
    ...
    state.rephrased_query = generate_text(prompt).strip()
